...
ax.text(0.01, 0.95, "KPI2: \u30a2\u30af\u30c6\u30a3\u30d9\u30fc\u30b7\u30e7\u30f3\u7387", transform=ax.transAxes,
...
ax.set_xlabel("週", fontsize=12)
ax.text(0.01, 0.95, "KPI3: \u9031\u6b21\u7d99\u7d9a\u7387", transform=ax.transAxes,
...
# Annotate latest
if c14b_total_all:
    ax14b.annotate(f"{c14b_total_all[-1]:,}", (_c14b_x[-1], c14b_total_all[-1]),
                   textcoords="offset points", xytext=(0, 8), ha="center",
                   fontsize=10, fontweight="bold", color="#333")
    ax14b_r.annotate(f"{c14b_users_all[-1]}", (_c14b_x[-1], c14b_users_all[-1]),
                     textcoords="offset points", xytext=(8, 5), ha="left",
                     fontsize=9, color="#1565C0")
...
ax14b.legend(lines1b + lines2b, labels1b + labels2b, loc="upper left", fontsize=8)

set_weekly_xticks(ax14b, c14b_weeks, equal_spacing=True)
fig14b.tight_layout()
fig14b.savefig(OUTPUT_DIR / "chart14b_weekly_search_volume_all.png", dpi=150, bbox_inches="tight")
print(f"[OK] Chart 14b -> output/chart14b_weekly_search_volume_all.png", flush=True)
if c14b_total_all:
...
<div class="summary">
  <div class="card" style="border-left:4px solid #42A5F5;">
    <div class="value" style="color:#1565C0;">{email_registered_total}</div>
    <div class="label">メール登録数（累計）<br><span style="font-size:11px;color:#aaa;">計画B必要数: {EMAIL_TARGET_REG_B:,}</span></div>
  </div>
  <div class="card" style="border-left:4px solid #E65100;">
    <div class="value" style="color:#E65100;">{MATURED_CONV_RATE*100:.0f}%</div>
    <div class="label">メール→医師 確定転換率<br><span style="font-size:11px;color:#aaa;">登録{MATURATION_WEEKS}週以上前 / 4週ローリング</span></div>
  </div>
</div>

<div class="summary">
  <div class="card purple">
    <div class="value">{mau_vals[-1]}</div>
    <div class="label">MAU（{disp_week_label}、D4+、28日窓）</div>
  </div>
  <div class="card pink">
    <div class="value">{mau_rate_vals[-1]:.1f}%</div>
    <div class="label">MAU率（MAU / 累計登録医師数）</div>
  </div>
  <div class="card cyan">
    <div class="value">{avg_dau_vals[-1]:.1f}</div>
    <div class="label">平均DAU（{disp_week_label}、D4+）</div>
  </div>
  <div class="card red">
    <div class="value">{dau_rate_vals[-1]:.1f}%</div>
    <div class="label">DAU率（平均DAU / 累計登録医師数）</div>
  </div>
</div>

<div class="summary">
  <div class="card green">
    <div class="value">{s15_count.get(exp_chart_weeks[-1], 0) if exp_chart_weeks else 0}</div>
    <div class="label">ヘビーユーザー数（28日間10回以上検索）<br><span style="font-size:11px;color:#aaa;">MAU中 {s15_pct_mau.get(exp_chart_weeks[-1], 0) if exp_chart_weeks else 0}%</span></div>
  </div>
  <div class="card blue">
    <div class="value">{s13_ret_avg.get(exp_chart_weeks[-1], 0) / max(s13_all_avg.get(exp_chart_weeks[-1], 0), 0.1):.1f}x</div>
    <div class="label">継続 vs 全WAU 検索深度比<br><span style="font-size:11px;color:#aaa;">継続ユーザーの検索数 / 全WAUの検索数</span></div>
  </div>
  <div class="card orange">
    <div class="value">{s16_wavg[1] if s16_wavg and len(s16_wavg) > 1 and s16_wavg[1] is not None else 0}%</div>
    <div class="label">アクティベーション後 M+1 リテンション<br><span style="font-size:11px;color:#aaa;">D4+初検索後30日間、加重平均</span></div>
  </div>
</div>
